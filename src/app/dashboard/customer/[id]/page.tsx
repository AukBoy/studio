...
export default function CustomerDetailPage() {
  ...
  }, [params.id, customers, isMounted]);

  const handleDelete = () => {
    setCustomers(customers.filter((c) => c.id !== params.id));
    toast({
      title: "Customer Deleted",
      description: `${customer?.name} has been removed from your records.`,
    });
  ...
  const handleMeasurementSave = (newMeasurementSet: MeasurementSet) => {
    if (!customer) return;

    const updatedCustomer: Customer = {
        ...customer,
        measurementSets: [...(customer.measurementSets || []), newMeasurementSet]
    };
  ...
  const handleUpdateStatus = (setId: string, newStatus: OrderStatus) => {
    if (!customer) return;

    const updatedMeasurementSets = customer.measurementSets.map(set => {
        if (set.id === setId) {
            const updatedSet = { ...set, orderStatus: newStatus };
            if (newStatus === 'Completed') {
                updatedSet.completionDate = new Date().toISOString();
            } else if (newStatus === 'Handed Over') {
                updatedSet.handoverDate = new Date().toISOString();
            }
            return updatedSet;
        }
        return set;
    ...
    const updatedCustomer: Customer = {
        ...customer,
        measurementSets: updatedMeasurementSets
    };
  ...
  const handleUpdatePaymentStatus = (setId: string, newStatus: PaymentStatus) => {
    if (!customer) return;

    const updatedMeasurementSets = customer.measurementSets.map(set => {
        if (set.id === setId) {
            return { ...set, paymentStatus: newStatus };
        }
        return set;
    });
